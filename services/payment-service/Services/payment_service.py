...
ORDER_SERVICE_URL = "http://order_service:5004/api"
TABLE_SERVICE_URL = "http://table_service:5006/api"
...
def create_invoice_service(data, token, user_role):
    ...
    try:
        order_res = requests.get(f"{ORDER_SERVICE_URL}/orders/{order_id}", headers=headers, timeout=5)
...
def get_invoice_detail_service(invoice_id, user_id, user_role, token):
    """Nghiệp vụ: Xem chi tiết hóa đơn (Có xác thực quyền của Customer)"""
    invoice = get_invoice_by_id(invoice_id)
    if not invoice:
        return {"success": False, "message": "Không tìm thấy hóa đơn"}, 404

    # Khách hàng chỉ được xem hóa đơn của chính mình
    if user_role == "CUSTOMER":
        headers = {"Authorization": f"Bearer {token}"} if token else {}
        try:
            order_res = requests.get(f"{ORDER_SERVICE_URL}/orders/{invoice['order_id']}", headers=headers, timeout=5)
            order_data = order_res.json().get("data", {})
            if str(order_data.get("customer_id")) != str(user_id):
                return {"success": False, "message": "Bạn không có quyền xem hóa đơn của người khác"}, 403
        except:
            return {"success": False, "message": "Lỗi xác thực quyền truy cập hóa đơn"}, 500
...
def process_payment_service(invoice_id, data, token, user_id, user_role):
    ...
    if user_role == "CUSTOMER" and method != "ONLINE":
    ...
    if user_role in ["CASHIER_LOBBY", "MANAGER", "ADMIN"] and method not in valid_staff_methods:
    ...
    transaction_id = data.get("transaction_id", f"TXN-{uuid.uuid4().hex[:6].upper()}")
    ...
    try:
        ...
        if order_res.status_code == 200:
            ...
            requests.put(f"{ORDER_SERVICE_URL}/orders/{order_id}/status", json={"status": "COMPLETED"}, headers=headers, timeout=5)
    ...
    return {"success": True, "message": f"Thanh toán ĐỦ qua {method}. Đơn hàng COMPLETED!"}, 200